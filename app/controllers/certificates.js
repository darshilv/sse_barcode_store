...
exports.create = function(req,res){
  console.log(req.files.activeImage.name + "||" + req.files.activeImage.path);
  //using the Synchronous method to avoid files being ignored   
  var cert = new Certificate();
  cert.name = req.body.name;  
  cert.activeImage.data = fs.readFileSync(req.files.activeImage.path);
  cert.activeImage.contentType = 'image/png';
  cert.inactiveImage = fs.readFileSync(req.files.inactiveImage.path);  
  cert.save(function(err){
    if(err) res.render('500');
    res.render('certificates/show', {title: 'New Certificate', certificate: cert});        
  });
